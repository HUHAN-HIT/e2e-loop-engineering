...
- **`clarification` 人锚点 (2026-06-30 回退 2026-06-28 演进)**: medium/complex run 在 CLARIFYING 阶段 dispatch clarification-finder 产 `clarification/questions.json` 后, 主 agent **必须**调 `e2e-loop submit-clarification <run_id>` —— 有阻塞性问题时 Coordinator `set human_pending=clarification`, 主 agent 用 AskUserQuestion 弹结构化框 (推荐选项 = `default_if_unanswered`) 问人, 用户答完调 `e2e-loop answer-clarification --answers <file>` 推进 PLANNING; 无阻塞 (空 questions + 非空 skip_basis) 不 set 锚点直接进 PLANNING。`guard_anchors` 在 CLARIFYING + `questions.json` 已存在但无锚点 时 deny; PLANNING + design.md/task-plan.yaml 已存在但无 `plan_signoff` 时 deny (堵主 agent 越权自写计划)。simple 档规则驱动跳过澄清 (`complexity=simple` 即证据)。实现于 `packages/ssot-ts/src/runtime/coordinator.ts` (`submitClarification` / `answerClarification`) 与 `packages/shared/src/hooks/guard_anchors/logic.ts` (`checkClarifyingPhase` / `checkPlanningPhase`)。
