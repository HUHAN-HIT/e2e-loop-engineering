...
- `e2e-loop` 是否在 PATH **不需要 `where`/`which` 现探** —— 不确定就直接用项目内入口 `node packages/cli/dist/index.js ...`。
