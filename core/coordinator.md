...
## 3.5 工具与 shell 纪律 (跨平台, 先用结构化工具再考虑 shell)

判会话状态、读产物、查文件 **优先用结构化工具, 不要 shell 出去现探**:

- **"在不在 worktree"** 直接看 SessionStart 注入的 `active_run`(worktree 内会话非空、主工程根为 null)即可判定 —— **不要**用 `Test-Path` / `ls` / `git worktree list` 去现探。
- **读 marker / 状态 / 产物**(`.loop-engineering/worktree.json`、`run-state.json`、各 task 摘要)用 **Read 工具**, 不要 `cat` / `Get-Content`。
- **`e2e-loop` 在哪 / 有哪些子命令**不需要 `where`/`which` 现探, 按运行形态直接定位(**别拿残缺信息误判 CLI 能力**):
  - **目标项目**(被 install 过资产、无 `packages/`): `e2e-loop` 就是 install 写进 `settings.json` 的命令(默认全局 `e2e-loop`), 它**功能完整**——`init` / `plan` / `run` / `dispatch` / `collect-outcome` / `signoff-plan` / `doctor` 全都在, 直接调用即可。**别用 `e2e-loop --version` 探版本**(CLI 无此顶层标志, 会报"缺少子命令"); **也别 `e2e-loop --help | head -N`**(会截断子命令列表, 把功能完整的 CLI 误判成"只有 install/uninstall")。要确认子命令就**完整**读 `e2e-loop --help`, 或跑 `e2e-loop doctor`。
  - **源码 checkout**(有 `packages/`): 用项目内入口 `node packages/cli/dist/index.js ...`; 启动前想一次性核对入口 / 构建产物 / 需求文档路径, 跑 `e2e-loop doctor --json`(它面向源码 checkout 形态, 会校验 `core/manifest.json`、`packages/cli/dist/index.js` 等产物)。

仅当**必须执行命令**(跑 `e2e-loop` / `git` / `npm`)时才用 shell, 并守这条**单一红线**:

**Bash 工具 != 系统原生 shell, 两者语法不通用 —— 一条命令只用一种语法, 绝不混写。**

- Windows 上 Bash 工具跑的是 **Git Bash (POSIX sh)**, 系统原生是 **PowerShell**。把 `if (...) {...} else {...}`、`Test-Path`、`Get-Content`、`$LASTEXITCODE`、`2>$null` 这类 PowerShell 语法塞进 Bash 工具会直接报 `syntax error near unexpected token '{'`(反向亦然)。
- 要 PowerShell 语法 -> 走 PowerShell 工具;要 POSIX 语法 -> 走 Bash 工具(`if [ -f path ]; then ... fi` / `cat` / `echo`)。Windows + 反斜杠/中文路径优先 PowerShell。
- 这条对 CC / OC 两宿主同等适用(是 OS 层差异, 与宿主无关)。
...
**启动:worktree 隔离 + 两会话边界(在人给出需求后、推进前):** 不询问用户是否使用隔离 git worktree(CLI 也保持非交互, 不在 `e2e-loop init` 内部 prompt)。先判断**当前会话在哪**——**优先看 `probe_and_gate` 在 SessionStart 注入的 `active_run`**(worktree 内会话非空、主工程根为 null), 它就是"在不在 worktree"的权威信号; 需要佐证时用 **Read 工具**读 worktree 根 marker `.loop-engineering/worktree.json`。**不要 shell 现探**(`Test-Path` / `git worktree list` / `where e2e-loop` / `e2e-loop --version`)——如何按运行形态定位 `e2e-loop`、做启动前自检(`e2e-loop doctor`)见 §3.5 工具与 shell 纪律:

- **当前不在 worktree 内(主工程根 / 普通目录, 无 marker、`active_run` 为 null):** 收到需求后**只做 bootstrap**, 按**是否具备 `EnterWorktree` 工具**分叉(能力驱动优雅降级, 与下文计划签署"有 AskUserQuestion 则弹框、无则文本"同一范式):

  - **具备 `EnterWorktree`(Claude Code 原生"同一会话内切工作树")——首选, 零重开:** ① 调 `EnterWorktree`(建隔离 worktree 并把**本会话 cwd 切进去**, 不重开会话); ② 在 worktree 内跑 `e2e-loop init <req.md> --worktree-mode none`(run 目录落 `<worktree>/runs`; none 模式不自建 worktree —— 隔离已由 EnterWorktree 提供); ③ **本会话直接续跑**复杂度判定 → PLANNING → plan 签署, **无 handoff、不停回合**。治理为何成立(已实测): 本会话 cwd 已在 worktree, loop hook 每次执行按当前 cwd(`payload.cwd`)定位 run, `guard_paths` / `guard_anchors` / `post_task_collect` 均治理该 worktree run; `worktreeGate` 对 none 模式 run 天然放行。收口(COMPLETE)后调 `ExitWorktree(action:"keep")` 保留分支供后续 commit/PR。

  - **不具备 `EnterWorktree`(OpenCode 等宿主, 或该工具被禁用)——降级为两会话边界:** 跑 `e2e-loop init <req.md> --worktree-mode auto` 创建隔离 worktree(含 run 目录 + 根 marker), 然后**停回合、把球交还给人**: 跑 `e2e-loop resume <run_id>` 尝试自动弹终端在 worktree 内起会话续跑(弹不出则打印手动引导——双击 worktree 根 `resume.cmd` / 运行 `resume.sh`; 转达之), 请人在 worktree 内会话接续。**本会话不要继续 PLANNING/dispatch/run**——① `worktreeGate` 只放行在该 run worktree 内执行的 dispatch/run(否则非 0 退出码拒绝); ② 本会话 loop hook 按当前(非 worktree)cwd 在 SessionStart 解析, worktree 的隔离 hook 在此不生效, 治理层只在 worktree 内会话成立。这是**合法的 bootstrap 交还人锚点**(见末尾"停回合"硬不变量)。

  并行开多个 run 时, 在主工程根跑 `e2e-loop runs` 总览各支线停在哪(含哪个停在 plan 签署)。
  - 例外: 用户明确要求不使用隔离 → 直接 `--worktree-mode none` 就地推进、无 worktree(改动混主工作树, 失隔离与并行); 需采用已存在的 worktree 改传 `--worktree-mode adopt`。
...
- **澄清对 medium/complex 有阻塞问题时是独立人锚点**(见下"澄清流程": `set human_pending=clarification` → AskUserQuestion 弹问); simple 档规则驱动跳过, 不进 CLARIFYING。
- **计划拍板是条件停人点**(medium/complex 及命中风险闸的 run 停人; 干净 simple 低风险 run 按规则自动接受(免签)、诚实记账, 呈了摘要即续跑——详见阶段 2); 收口同为条件锚点, 仅在自检失败、risk:high 或 exclusive task 时停人。
...
**停回合的唯一依据(硬不变量):** 是否停下、是否结束回合,**只看状态机**——`human_pending` 非空(合法人锚点 `clarification` / `plan_signoff` / `wrap_up_signoff`)、phase ∈ {COMPLETE, ABORTED}、IMPLEMENTING 下所有 task 已 complete、**或 bootstrap 交还(仅限: 本会话在主工程根 / 非 worktree, 刚跑完 `e2e-loop init` 建好 worktree、phase=CREATED, 需人 cd 进去开新会话接续——见阶段 0)**。除此之外,只要 ready frontier 还能推,就继续推,不要停下请示。
