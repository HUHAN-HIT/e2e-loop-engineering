...
- **当前不在 worktree 内(主工程根 / 普通目录, 无 marker、`active_run` 为 null):** 收到需求后**只做 bootstrap**——跑 `e2e-loop init <req.md> --worktree-mode auto` 创建隔离 worktree(含 run 目录 + 根 marker)。init 成功后**立即跑 `e2e-loop resume <run_id>`**——它自动弹出一个新终端、在该 worktree 内起会话并以 `/loop-engineering` 首条消息触发 skill 续跑(SKILL 检测 `active_run` 非空即走下条"已在 worktree 内→续跑"分支, 一路推进到 plan 签署)。**人零操作, 只需在弹出的窗口里等 plan 签署拍板**; 随后**本会话停回合、把球交还给人**(resume 若未能弹出终端会打印手动引导——双击 worktree 根 `resume.cmd` / 运行 `resume.sh`; 转达之)。并行开多个 run 时, 在主工程根跑 `e2e-loop runs` 总览各支线停在哪。**本会话不要继续 PLANNING/dispatch/run**——原因有二: ① CLI 的 `worktreeGate` 只放行在该 run worktree 内执行的 dispatch/run(否则非 0 退出码拒绝); ② 本会话的 loop hook 是按当前(非 worktree)cwd 在 SessionStart 解析的, worktree 的隔离 hook 在此不生效, 治理层(guard_paths / guard_anchors / post_task_collect)只在 worktree 内会话成立。这是一个**合法的 bootstrap 交还人锚点**(见末尾"停回合"硬不变量)。
