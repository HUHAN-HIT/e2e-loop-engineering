...
3. **人锚定质量。** 质量的锚首先是你在**计划拍板**时冻结意图；收口只在自检失败或高风险时交还给你判断, 不是一堆 agent 互审。普通全绿自动完成。
...
本范式把质量锚在人的判断上 (设计原则 #3), 因此**人的注意力是系统最稀缺的资源**, 会被分散稀释。当前需要人的点分为必经与异常: 计划拍板必经; 收口看 diff 仅异常/高风险触发; 契约变更、risk 判定、complexity 判定尽量机制前移。全部平摊给人, 短期可行, 长期每个点都看不仔细。
...
| **条件收口验收** —— 自检失败或高风险时整体是否接受, 含 `key-diffs.md` 逐条 (§2.3) | **complexity 判定** —— 规则给初值 (AC 数 / 服务数 / 任务数), 人只在边界 case 调整 |
|  | **契约变更** —— `service-contracts.yaml` diff 机制判定 (§11.2), 人不盯 |

取向: 把"需要人"的点尽量前移成"机制判定 + 异常上报", 只把真正需要人类意图判断的计划拍板与异常/高风险收口留给人。**新增任何"需要人看"的环节前, 先问它能否降级为"机制判定 + 只在异常时报人"; 能, 就不要再占用注意力预算。** 这是一条持续设计约束: 每加一个判断点都要过这道筛子, 否则注意力会被慢慢蚕食回对抗式那种"处处要盯"。
...
`human_pending` 取值为 `null`、`"plan_signoff"` 或 `"wrap_up_signoff"` —— 协作式核心字段: 系统只在这个值非空时把球交给你。`plan_signoff` 必经; `wrap_up_signoff` 仅在收口自检失败、`risk: high` 或 `exclusive: true` task 时设置。
