...
人介入分两类, 由 `run-state.human_pending` 标记:

- **信息请求** (CLARIFYING 期, 可选, `human_pending == "clarification"`): 仅当 simple/medium 档实在无法做默认假设时触发; 多数 run 不经过。
- **质量锚定** (两处, 必经, 设计原则 #3 所指):
  1. **计划拍板** (PLANNING 末, `"plan_signoff"`): "设计、拆分和测试设计如下, 是否补充或修改?" —— 质量最大杠杆在这里。
...
runs/<run_id>/
  ...
  clarification/        # 可空 (simple 档跳过)
    questions.json      # finder 产出: 阻塞性问题 + 各自默认假设
    answers.json        # 人回答后由 coordinator 写入; 人不答则记录"采用默认"
  planning/
    design.md
    task-plan.yaml
...
`human_pending` 取值为 `null`、`"clarification"`、`"plan_signoff"` 或 `"wrap_up_signoff"` —— 协作式核心字段: 系统只在这个值非空时把球交给你。三类非空值分别对应 §1 中人介入的三类时机。
...
2. clarification (单 agent, 可跳过; 完成条件 = 无阻塞性歧义, 或 `human_pending="clarification"` 已被回答) + 计划拍板
