...
| **P1** | npm workspace 通; `@e2e-loop/cli` + `@e2e-loop/adapter-cc` (CC adapter 全 TS, 含 4 hook TS 重写); 替换原 `e2e-loop install-claude` | 3 天 | P1 hook 等价测试通过 | 0.3.0-alpha (npm 装 CC adapter 可用) |
| **P2** | `@e2e-loop/adapter-oc` 基础: SKILL.md 落 `.claude/skills/` (OC 直接读) + subagent 双 frontmatter | 1.5 天 | OC 主 agent 能加载 SKILL.md 并跑通 simple run | 0.4.0-alpha (OpenCode 降级可用) |
...
1. ✅ §14 决策已全部拍板 (v0.2);
2. P0 启动前事实核验: `npm view @e2e-loop/cli` 与 `npm view e2e-loop` 查重;
3. 启动 P0 (Monorepo 重构): 拆 `core/`, 建 `packages/` 与 `adapters/` 骨架;
4. P1 前确认 changesets 工作流;
5. P3 spike 启动前再读 `https://opencode.ai/docs/plugins/` (OpenCode plugin 文档, 本设计写作时尚未完整 fetch)。
