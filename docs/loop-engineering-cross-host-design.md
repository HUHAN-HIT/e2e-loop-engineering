...
2. **npm 化**: Node 写 installer 与 CLI, 通过 `loop-eng install --host <cc|oc|both>` 落资产。
...
- Claude Code 资产 (skill/agents/hooks/settings.json) 打包在 Python 包内, 通过 `loop-eng install-claude` 同步到目标项目 `.claude/`。
...
- F3: `loop-eng` CLI 跨平台, Windows/macOS/Linux 一致。
...
│   ├── cli/                               # @loop-eng/cli (loop-eng 命令)
│   ├── adapter-cc/                        # @loop-eng/adapter-claude-code
│   ├── adapter-oc/                        # @loop-eng/adapter-opencode
│   ├── shared/                            # @loop-eng/shared (跨 adapter 工具, 如 actual_writes 采集)
│   └── ssot-ts/                           # @loop-eng/ssot  (TS 实现的算法 SSOT, 见 §9)
│
...
│                                                              │
│  @loop-eng/cli ──┬──> @loop-eng/adapter-cc                   │
│                  ├──> @loop-eng/adapter-oc                   │
│                  ├──> @loop-eng/shared                       │
│                  └──> @loop-eng/ssot (TS, 渐进填充)          │
└─────────────────────────────────────────────────────────────┘
                            │
                            │  (共存期)
                            ▼
...
─────                  ─────                   ─────                  ─────
core/*.md   ─┐                                   
adapters/   ─┼─> npm publish @loop-eng/*  ─> npm install -g      ─> 宿主加载 .claude/ 或 .opencode/
packages/   ─┘   (workspace 自动串包)        loop-eng install        触发 hook / 加载 SKILL.md
                                              --host <cc|oc|both>
                                              --project-dir <p>
```
...
import { handleGuardPaths } from "@loop-eng/shared";
...
- `fs` 写权限探测: 试写 `.claude/.loop-eng-probe` 临时文件后删除。
...
- 全部 M1-M7 完成后, 在下一个大版本 (例如 1.0.0):
  ...
  - 删除 `pyproject.toml` 的 `[project.scripts] loop-eng` 入口;
  - 仅保留 `package.json` 入口;
...
```bash
# 安装 (核心命令)
loop-eng install --host <cc|oc|both> --project-dir <path> [--force]

# 预览 (不写盘)
loop-eng install --host <cc|oc|both> --project-dir <path> --dry-run

# 卸载
loop-eng uninstall --host <cc|oc|both> --project-dir <path>

# 列出当前项目已装的 adapter 资产
loop-eng list --project-dir <path>

# 算法 SSOT 本地 dry-run (兼容旧 Python CLI, M7 后由 TS 接管)
loop-eng init <requirement.md>
loop-eng status <run_id>
loop-eng plan <run_id> --design <file> --task-plan <file>
loop-eng run <run_id>
loop-eng wrap-up <run_id>
loop-eng signoff-plan <run_id> [--reject --feedback <text>]
loop-eng signoff-wrap-up <run_id> [--reject]
loop-eng abort <run_id> --reason <text>
loop-eng amend <run_id> --reason <text> --ac <id>...

# 跨宿主等价性验证 (CI 用)
loop-eng verify --fixture <fixture-dir>
```
...
| --- | --- | --- | --- | --- |
| **P0** | Monorepo 重构: 拆 `core/`, 建 `packages/` 与 `adapters/` 骨架, 现有 Python 资产迁入 `core/` | 1.5 天 | — | 0.2.0-alpha (Python CLI 不变, 仅仓库重构) |
| **P1** | npm workspace 通; `@loop-eng/cli` + `@loop-eng/adapter-cc` (CC adapter 全 TS, 含 4 hook TS 重写); 替换原 `loop-eng install-claude` | 3 天 | P1 hook 等价测试通过 | 0.3.0-alpha (npm 装 CC adapter 可用) |
| **P2** | `@loop-eng/adapter-oc` 基础: SKILL.md 落 `.claude/skills/` (OC 直接读) + subagent 双 frontmatter | 1.5 天 | OC 主 agent 能加载 SKILL.md 并跑通 simple run | 0.4.0-alpha (OpenCode 降级可用) |
| **P3** | **OpenCode plugin spike**: 4 个 OC plugin 等价实现, 重点 `post_task_collect` | 4-5 天 | §8.4 四个 spike 问题有可行答案 | 0.5.0-alpha (OpenCode 全功能) |
...
import { parseCheck, evalCheck } from "@loop-eng/ssot/checklists/checks_eval";
...
以下决策点不影响本设计书成立, 但影响后续实施细节。请勾选:

### D-OPEN-1: npm 包结构

- [ ] **A. Monorepo + 子包** (本设计推荐): `@loop-eng/cli`, `@loop-eng/adapter-cc`, `@loop-eng/adapter-oc`, `@loop-eng/shared`, `@loop-eng/ssot`。优势: 可独立引用; 劣势: 发版复杂。
- [ ] **B. 单包**: `loop-engineering` 一个包, 入口暴露所有 adapter。优势: 发版简单; 劣势: 用户装 OC 也要下 CC 代码。
- [ ] C. 其他

### D-OPEN-2: Python SSOT 最终归宿

- [ ] **A. 完全废弃** (本设计推荐): 1.0.0 删除 Python 包, 仅 TS。
- [ ] B. 长期双轨: Python 作为参考实现保留, 不删。
- [ ] C. Python 仅保留 schema 子包 (作为 Pydantic 等价参考), 其他全迁。

### D-OPEN-3: 共存期版本同步策略

- [ ] **A. changesets + 手动同步 PyPI** (本设计推荐)。
- [ ] B. 自动化: GitHub Action 监听 npm publish 后自动 PyPI publish。
- [ ] C. 仅 minor 同步, patch 独立。

### D-OPEN-4: TS 运行时选型

- [ ] **A. Bun** (本设计推荐): 启动快, 测试一体, 但要求用户机器有 Bun 或打包成单文件。
- [ ] B. Node 20+: 兼容性最好, 但 hook 冷启动慢 (200ms+)。
- [ ] C. Deno: 兼容性中等, 内置 TS。

### D-OPEN-5: OpenCode plugin 实现语言

- [ ] **A. TS (与 CC adapter 一致)** (本设计推荐)。
- [ ] B. 各 adapter 用宿主原生最简栈 (OC 用 TS, CC 用 TS, 已经一致)。

### D-OPEN-6: 文档/品牌统一

- [ ] **A. 保留 "loop-engineering" 名字** (本设计推荐), npm scope 用 `@loop-eng`。
- [ ] B. 改名 (因 npm 上 `loop-engineering` 可能已被占, 需在 P0 阶段查重)。
...
1. 用户勾选 §14 的开放决策点;
2. 启动 P0 (仓库重构);
3. P1 前确认 §10.2 的 changesets 工作流;
4. P3 spike 启动前再读一遍 `https://opencode.ai/docs/plugins/` (OpenCode plugin 文档, 本设计写作时尚未完整 fetch)。
