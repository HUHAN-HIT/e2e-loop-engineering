...
| **P4** ✅ | Python SSOT → TS 迁移: M1-M6 (schema→state_machine/checks_eval→scheduling→checklists→amendment/multi_service→trust_mode), 全落 `packages/ssot-ts` (zod), 复用 shared 的 matchPath/actual_writes | 5-6 天 | 每子包等价测试通过 (tests-ts/ssot/ 271 条, 全套 432 pass; pytest 295 无回归) | 0.6.0 - 0.9.0 (渐进) **已完成 2026-06-28** |
| **P5** ✅ | M7 runtime/dispatch/cli 迁移 (→ packages/ssot-ts + cli dry-run 子命令); Python 包标记 deprecated; 文档切 npm (README + CLAUDE.md); CC/OC 跨宿主一致性 e2e | 2-3 天 | 全量通过: bun 468 pass, pytest 295 无回归; cross_host_consistency e2e (CC deny ⟺ OC throw) 绿 | 1.0.0 (Python 已归档到 archive/, 本地保留不进远程; 物理删除留待正式 1.0.0) **已完成 2026-06-28** |
...
7. ✅ P4 已完成 2026-06-28: Python SSOT → TS 迁移 M1-M6 全部落 `packages/ssot-ts` (zod 版 schema + state_machine + scheduling + checklists(含 checks_eval 递归下降解析器) + amendment + multi_service + trust_mode); scheduling 复用 shared 的 matchPath/actual_writes 不重复造轮子; tests-ts/ssot/ 271 条等价测试 (用例同源 Python tests/), 全套 432 pass, pytest 295 无回归。根 index 扁平导出 schema + 命名空间导出其余子包 (规避同名冲突)。
8. ✅ P5 (M7 + 收尾) 已完成 2026-06-28: M7 runtime (Coordinator 单写者 / tick 纯函数固定顺序 / directory 原子写重试) + dispatch (packet/collect/worker_runner echo 占位) 迁到 `packages/ssot-ts`; TS CLI 加 9 个算法 dry-run 子命令 (init/status/plan/run/wrap-up/signoff-plan/signoff-wrap-up/abort/amend) 接 TS runtime; Python 包标记 deprecated (pyproject + __init__, **未删代码**); 文档切 npm (新建根 README + CLAUDE.md 命令族); 新增 cross_host_consistency e2e (CC deny ⟺ OC throw, 共享 SKILL.md 字节一致)。全套 bun 468 pass, pytest 295 无回归, tsc 干净, 三 bundle 构建成功, CLI dry-run 端到端跑通。
9. ✅ Python 归档已执行 2026-06-28 (用户决策): `loop_engineering/` + `tests/` + `pyproject.toml` 移到 `archive/python-ssot/` (本地保留, 已 gitignore, **不进远程仓库**), 未物理删除。TS SSOT (packages/ssot-ts) 成为唯一在库权威; Python 作本地参考/回归锚点。TS 集成测试夹具已自包含到 `tests-ts/fixtures/smoke/` (不再依赖归档树)。
10. **剩余 (未来 1.0.0)**: 正式版本号切 1.0.0 + 本地物理删除 archive/ (D-2/§9.5 "完全废弃" 的收尾), 由用户在需要时执行。当前 P0-P5 全部完成, 跨宿主设计已落地。
