...
| **P5** ✅ | M7 runtime/dispatch/cli 迁移 (→ packages/ssot-ts + cli dry-run 子命令); Python 包标记 deprecated; 文档切 npm (README + CLAUDE.md); CC/OC 跨宿主一致性 e2e | 2-3 天 | 全量通过: bun 468 pass, pytest 295 无回归; cross_host_consistency e2e (CC deny ⟺ OC throw) 绿 | 1.0.0 (物理删除 Python 待确认后执行) **M7+收尾已完成 2026-06-28** |
...
9. **剩余 (待用户确认的不可逆动作)**: 物理删除 `loop_engineering/` Python 包 + `pyproject.toml` 入口, 正式出 1.0.0 (D-2/§9.5)。此为破坏性删除 (移除 ~3800 行参考实现 + 295 测试锚点), 需显式确认后执行; 在此之前 Python 以 deprecated 状态共存, 仍作等价测试锚点。
