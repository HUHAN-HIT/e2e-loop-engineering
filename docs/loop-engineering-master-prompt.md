...
3. **人锚定质量。** 质量的最终锚点是人在**计划拍板**时冻结意图；收口仅在自检失败或高风险时交还给人。普通全绿自动完成。(方法论演进 2026-07-01: 计划拍板对 `complexity=simple` 且未触发风险闸、未强制 `require_plan_signoff` 的 run 降级为条件锚点——`plan_check` 通过即自动接受(免签)进实施, 写 `planning/plan-auto-accepted.json` 诚实记账, 绝不记为人工签署; medium/complex 及命中风险闸的 run 仍必经拍板。)
...
> 例外(方法论演进 2026-07-01): 当 run `complexity=simple` 且未触发风险闸(`risk: high` / `exclusive: true` / 存在 service-contracts)、未强制 `require_plan_signoff` 时,此人盯点降级为条件锚点——`plan_check` 通过后**自动接受(免签)**进入实施,写 `planning/plan-auto-accepted.json` 诚实记账(**绝不记为人工签署**),不停下等人。medium/complex 及命中风险闸的 run 保留必经拍板。
