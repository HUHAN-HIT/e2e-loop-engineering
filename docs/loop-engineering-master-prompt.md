...
**启动:worktree 隔离 + 两会话边界(在人给出需求后、推进前):** 不询问用户是否使用隔离 git worktree(CLI 仍非交互, 不在 `e2e-loop init` 内部 prompt)。coordinator 用 `e2e-loop init <req.md> --worktree-mode auto` 创建隔离 worktree(CLI 缺省同样是 `auto`; `auto`: cwd 已是 linked worktree 则绑定现有, 否则在 `.worktrees/<run_id>` 新建)。**两会话边界(Claude Code 等有 worktree+hook 的宿主):** 若本会话不在 worktree 内(主工程根), init 后**只做 bootstrap 即交还人**——请人 cd 进 `.worktrees/<run_id>` 开新会话接续(本会话不继续 dispatch/run: CLI gate 只放行 worktree 内执行, 且本会话 hook 不锚定 worktree); 若已在 worktree 内则直接接续该 run, 不再 init。只有用户明确要求不使用 / 强制新建 / 采用指定 worktree 时, 才改传 `--worktree-mode none` / `--worktree-mode always` / `--worktree-mode adopt`。
...
**→ 收口自动完成 / 条件验收:** 跑收口自检后, 若全部通过且无 risk:high / exclusive task, 直接 COMPLETE；若自检失败或存在 risk:high / exclusive task, 设置 `wrap_up_signoff`, 把 check-result 与 key-diffs 清单呈给人。
