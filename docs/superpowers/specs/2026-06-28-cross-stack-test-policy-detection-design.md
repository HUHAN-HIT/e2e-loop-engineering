...
- **`task_check` 兼容性 (关键)**: 无 profile 时**不加入 `style_compliance` 项** (而不是加入但 pass), 保持现有 `items` 数量不变, 让现有 TS 测试断言 (`items.length === 4` 类) 不破。验收 #1 (无 profile 行为不变) 在此落实。
...
3. 新增 controller test 是否满足 required imports (MockMvc)。
4. 新增 annotation 是否触犯 forbidden (例如 JUnit4 `@RunWith`)。
5. 新增代码是否触犯 forbidden patterns (精确正则, 见精度约束)。
...
4. component test (按 planned case 类型判定) 是否使用了 required imports。
...
1. **command-evidence.json 文件存在性** = strict (文件缺失 = fail)。
2. **command 字段匹配 required_commands** = strict (命令本身要对)。
...
**无 profile 时**: 完全不加入此项, 保持现有 4 项不变 (兼容性关键, 验收 #1)。
...
- **strict 字段** = `schema`、`task_id`、`command`、`cwd`、`stdout_path`、`stderr_path`、`exit_code` (字段存在性与类型)。这些是文件级客观事实, 缺失即 fail。
...
- `collected_by: worker_self_report` 明示采集源, 与 design §0.5 风格一致; 未来升 `unattended` 档时改为 `coordinator_independent_replay`, schema 不动, B → A 平滑升级。
...
1. **没有 style profile 的现有 run 行为不变**: `task_check` 不加入 `style_compliance` 项, `items` 数量与现有完全一致 (兼容性测试断言不破)。
...
- **`post_task_collect.py` 必需 artifact 清单加入 `command-evidence.json` (仅 profile 存在时)**。
- **`task_check` 实现 "无 profile 不加项" 兼容性**。
- **`guard_paths.py` 永远 deny `style-profile.yaml` 写入**。

### Phase 3: JS/TS adapter MVP (首版核心)

- 实现 `diff_plus_extractor.ts` (git diff + 行抽取, 复用 actual_writes 基础设施)。
- 解析 `package.json` 的 diff + 行 (新增依赖)。
- 扫描测试文件 diff + 行的 import 行与 skip/only 正则。
- 检查 command evidence 的 strict 字段 (文件存在、command 匹配)。
- runner/coverage 字段 advisory 处理。
- 接入 task check。
- **关键回归: 历史 legacy 违规不 fail。**
...
| 风险 | 缓解 |
| --- | --- |
| **历史 legacy 违规被算到新 task 头上 (迁移期大爆炸)** | **strict 限定到 git diff + 行; 全仓库扫降级为 baseline advisory; profile 提供 `baseline_mode` 字段让团队显式选迁移策略** |
...
| **task_check 加项破坏现有测试** | **无 profile 时不加项, 保持 items 数量; profile 存在时才加第 5 项** |
