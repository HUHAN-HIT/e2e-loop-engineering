...
| 退化 | gh 优先,逐级退化 commit→push→PR,停在上一步成功处并如实报告 | 红线"诚实高于合规外观";只支持 gh(GitHub) |
...
- `--pr-draft` 缺省 true(draft PR);`--no-pr-draft` 显式建正式 PR。
...
- `channel.ts` — `probeFinalizeChannel(workdir, seams?)` → `FinalizeChannel{ has_remote, is_github, gh_ready }`。逐级探测:`git remote get-url origin`(有无 remote)、URL 是否 github.com、`gh auth status`(已安装+已认证)。任何异常吞掉返回保守 false。seam 可注入。
- `plan.ts` — `planFinalize(input)` → `FinalizePlan{ head_branch, base_ref, need_create_branch, do_commit, do_push, do_pr, pr_draft, downgraded_from, downgrade_reason }`。**统一 loop/ 分支 + 逐级退化的全部决策都在此纯函数内**。输入:binding(含 mode/branch/base_ref)、当前分支、`finalize_policy`、`pr_draft`、channel、commit 清单是否非空。
...
- PR:finalize-result 已有 `pr_url`,或 `gh pr view <head>` 已存在 → skip 建 PR(可选更新正文)。
...
export const FinalizeResultSchema = z.object({
  ...
  pr_draft: z.boolean(),
  compare_url: z.string().nullable(),   // 退化时给人接手的 URL
...
| 前置不满足 | achieved | 给人的接手信息 |
| --- | --- | --- |
| `policy=off` | `off` | 不做发布 |
| commit 清单为空 | `no_changes` | 无代码改动,未建分支/PR |
| 无 remote / push 失败 | `commit` | 已本地 commit 到 `<head>`,请手动 push |
| remote 非 GitHub 或 gh 未就绪 | `commit_push` | 已推 `<head>`,打印 compare URL 供手动建 PR |
| 全部就绪 | `full_pr` | 返回(draft)PR URL |

任一退化都写 `downgrade_reason` + 必要的 `compare_url`,**绝不在 errors 为非空时谎报 `full_pr`**。
...
2. 收口后确定性执行 commit(actual_writes 并集)→ push(loop/ 分支)→ 建 draft PR。
...
- 不支持 GitHub 以外的 PR 后端(glab/web)——YAGNI。
