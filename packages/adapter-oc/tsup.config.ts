/**
 * adapter-oc 的 OpenCode plugin 构建配置。
 *
 * 把 src/plugin/index.ts 连同 @e2e-loop/shared (及其它非 node 内置依赖) bundle 成
 ...
 *   - adapter-cc 产 4 个 hook .mjs (CC stdin/stdout 形态); 本配置产 1 个 plugin .js (OC plugin 形态)。
 *   - 输出扩展名用 .js (OC 文档示例用 .js/.ts; .js 更通用), 不是 .mjs。
 *   - @opencode-ai/plugin 仅类型, plugin 源码用 import type / 自写最小类型, 不进 bundle。
 *
 * install.ts / index.ts / render.ts 仍由 CLI 通过 workspace import 直接用源码, 不需独立产物;
 * 本配置只产出 plugin 单文件。
 */
