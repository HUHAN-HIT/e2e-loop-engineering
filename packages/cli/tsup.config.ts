 ...
 * CLI 构建配置。
 *
 * 把 src/index.ts bundle 成单文件 dist/index.js:
 ...
 *     ./dist/index.js 或 npm install -g; 这里不再用 banner 注入, 否则会出现
 ...
 * `node cli/dist/index.js install --host oc` 独立运行会因找不到 js-yaml 而报错。
 *
 ...
 * 为让 `node cli/dist/index.js init ...` 独立可跑 (无需在 cli/dist 旁装 node_modules),
